 ...
 */
public class HdfsFile implements FtpFile {

	private final Logger log = LoggerFactory.getLogger(HdfsFile.class);
	 ...
	 */
	public HdfsFile(String path, User user) {
...
//        }
	...
	private HdfsFile getParent() {
		...
		return new HdfsFile(parentS, user);
	}
	...
	public List<FtpFile> listFiles() {
		...
		try {
			...
			for (int i = 0; i < fileStats.length; i++) {
				fileList.add(new HdfsFile(fileStats[i].getPath().toString(), user));
			}
			return fileList;
		} catch (IOException e) {
			log.debug("", e);
