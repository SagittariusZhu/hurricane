...
public final class DateTool {  
	public static SimpleDateFormat hurricaneDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SS'Z'");
	public static SimpleDateFormat[] LUCENE_DATE_FORMATS = {
			new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'"),
			new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SS'Z'")
	};
	public static String defaultTZ = "UTC";
     ...
     * @return 转化后的日期时间. 
     */  
    public static Date date2Timezone(Date srcDateTime, String srcTimeZoneId,  String dstTimeZoneId) {  
        ...
        if (dstTimeZoneId == null || "".equals(dstTimeZoneId))  
            return null;  
        int srcTimeToUtc = getTimeZoneRawOffset(srcTimeZoneId);
        int dstTimeToUtc = getTimeZoneRawOffset(dstTimeZoneId);           
		long nowTime = srcDateTime.getTime();  
		long newNowTime = nowTime - srcTimeToUtc + dstTimeToUtc;  		
    ...
    /**
     * Called by QueryParse.
     * @param parse
     * @return
     */
	public static String dateToString(Date parse) {
		return hurricaneDateFormat.format(parse);
	}
    

	/**
	 * Parse Lucene format data string
	 * @param value
	 * @return
	 * @throws ParseException 
	 */
	public static Date string2Date(String value) throws ParseException {
		for (SimpleDateFormat sdf : LUCENE_DATE_FORMATS) {
			try {
				Date ret = sdf.parse(value);
				return ret;
			} catch (ParseException ignore) {}
		}
		throw new ParseException("Unparse " + value, 100);
	}
