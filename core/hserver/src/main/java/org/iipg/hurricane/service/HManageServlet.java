...
import org.iipg.hurricane.service.cmd.ListCmd;
import org.iipg.hurricane.util.ServletUtil;
import org.json.JSONException;
import org.json.JSONObject;


public class HManageServlet extends HttpServlet implements HCommandConstants {

	public static final String SERVLET_NAME = "HurricaneManagerServlet";
	...
	throws ServletException, IOException
	{
		...
		if ("json".equals(req.getParameter("wt"))) {
	        ...
	        res.setStatus(header.getInt(KEY_STATUS));
	        try {
			...
			JSONObject header = command.getResponseHeader();
			if (header.has(KEY_CONTENT_TYPE)) {
				res.setContentType(header.getString(KEY_CONTENT_TYPE));
			} else {
				res.setContentType("text/plain;charset=utf-8");				
			}
			
	        res.setStatus(header.getInt(KEY_STATUS));
	        res.getWriter().println(command.getRawOutput());			
		}
	}
}
