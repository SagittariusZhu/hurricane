...
var solr_admin = function( app_config )
{
  ...
  {
    var cores = sort_by_key(cores_status, "name");

    return cores;
  };

  sort_by_key = function sort_by_key(array, key) {
  ...
  {
    //暂不进行排序
    //that.cores_data = cores.data.cores;

    that.cores_data = sort_cores_data( cores.data.cores );
    that.menu_element
      .empty();
    ...
    var core_count = 0;
    for( var i=0;i<that.cores_data.length;i++ )
    {
      core=that.cores_data[i];
      core_count++;
      var core_path = config.solr_path + '/' + core.name;
      var classes = [];
  ...
  this.set_sources_data = function set_sources_data( sources )
  {

    that.sources_data = [
              {name:"MysqlClusterA", desc:"MySQL (4 Nodes)", dbType:"mysql", connectURL:"jdbc:mysql:localhost@aaa/pass:test"}, 
              {name:"SolrCloudA", desc:"Solr (4 Nodes)", dbType:"oracle", connectURL:"jdbc:oracle:localhost@bbb/pass:test"}
    ];

  };

  this.get_instance_data = function get_instance_data( cur_source, sources )
  {

    for (var idx in sources) {
      var source = sources[idx];
      if (cur_source == source.name) {
        return source;
      }
    }
    return {};
  ...
  this.run = function()
  {
    ...
    (
      {
        url : config.url+config.solr_path + config.core_admin_path + '?wt=json&action=LIST',
        dataType : 'json',
        async: false,
              ...
              function( event )
              {
                ...
                that.core_menu
                  ...
                  (
                    ...
                    '<li class="datasync"><a href="#/' + core_name + '/datasync"><span>DataSync</span></a></li>' + "\n" +
                    '<li class="schema-update"><a href="#/' + core_name + '/schema-update"><span>Schema Manager</span></a></li>' + "\n" +
                    //'<li class="subscribe"><a href="#/' + core_name + '/subscribe"><span>Subscribe</span></a></li>' + "\n" +
    ...
    var system_url = app_config.url+app_config.solr_path + '/admin/system?wt=json';
    $.ajax(
      {
              url : system_url,
              dataType : 'json',
  ...
  this.format_xml = function format_xml( xml_str )
  {
      var xml = $.parseXML(xml_str);
      var jsonObj = this.xmlToJson(xml);
      return JSON.stringify(jsonObj, undefined, 2).esc();
  };

  // Changes XML to JSON
  this.xmlToJson = function xmlToJson(xml) {
    ...
    } else if (xml.nodeType == 3 && $.trim(xml.nodeValue.replace("\n","")) !== "") { // text
      obj = xml.nodeValue;
    }
