    ...
    (
      {
        ...
        success : function( response, text_status, xhr )
        {
          ...
          var has_cores = false;
          for( core in response.data.cores )
          {
            has_cores = true; 
            break;
          }
...
(
  ...
  function( event, params )
  {
    ...
    for( var i=0;i<params.cores.length;i++ )
    {
      core=params.cores[i];
      var core_name = core.name;
      if( !core_name )
      {
        core_name = '<em>(empty)</em>';
      }
      navigation_content.push( '<li><a href="' + params.basepath + core.name + '">' + core_name + '</a></li>' );
    }
...
(
  ...
  function( event, params )
  {
    ...
    (
      {
        url : config.url+config.solr_path + app.config.core_admin_path + '?wt=json&action=property&name=' + params.current_core,
        dataType : 'json',
        beforeSend : function( xhr, settings )
        {
        },
        success : function( response, text_status, xhr )
        {
          ...
          if( has_cores )
          {
            params.success( response.data.cores );
          }
        },
        error : function( xhr, text_status, error_thrown)
        {
    ...
    (
      'cores_load_data',
      {
        success : function( cores )
        {
          ...
          context.redirect( context.path + '/' + first_core.name );
        },
        error : function()
        {
          sammy.trigger
              ...
              callback : function()
              {
                ...
                (
                  'cores_load_property',
                  {
                    basepath : path_parts[1],
                    current_core : current_core,
                    core_data_element : core_data_element,
                    success : function( cores )
                    {

                      var coreData = cores[0]['core'];
                      for( var key in coreData )
                      {                                                        
                        var value_element = $( '.' + key + ' dd', core_data_element );

                        if (typeof coreData[key] == "string")
                          value_element
                            .text( coreData[key].esc() );
                        else if (typeof coreData[key] == "boolean") {
                          value_element
                            .addClass( coreData[key] ? 'ico-1' : 'ico-0' );
                        } else
                          value_element
                            .text( coreData[key] );
                      ...
                      var indexData = cores[0]['index'];
                      for( var key in indexData )
                      {                                                        
                        var value_element = $( '.' + key + ' dd', index_data_element );

                        if (typeof indexData[key] == 'string')
                          value_element
                            .text( indexData[key].esc() );
                        else if (typeof indexData[key] == 'boolean') {
                          value_element
                            .addClass( indexData[key] ? 'ico-1' : 'ico-0' );
                        } else
                          value_element
                            .text( indexData[key] );
                      ...
                      var indexStatusData = coreData.data.indexStatus;
                      $( '.lastModified dd', index_data_element )
                          .html( indexStatusData.lastModified || '-' );
                      $( '.deletedDocs dd', index_data_element )
                          .html( indexStatusData.deletedDocs || '-' );                          
                      $( '.optimized dd', index_data_element )
                          .addClass( !indexStatusData.hasDeletions ? 'ico-1' : 'ico-0' );                          
                    }
                  }
                );
                ...
                reload_button
                  ...
                  (
                    ...
                    function( event )
                    {
                      ...
                      (
                        {
                          url : app.config.url+app.config.solr_path + app.config.core_admin_path + '?wt=json&action=REFRESH&name=' + current_core,
                          dataType : 'json',
                          context : $( this ),
                          beforeSend : function( xhr, settings )
                          {
                ...
                $( '#actions #unload', cores_element )
                  ...
                  (
                    ...
                    function( event )
                    {
                      var ret = confirm( 'Do you really want to delete Core "' + current_core + '"?' );
                      if( !ret )
                      {
                        return false;
                      }
